...
int read_fasta(struct in_buffer* b, struct msa** msa, int number_of_seqs, char** names_of_sequences, char** sequences_in_cluster);
...
int main(int argc, char *argv[])
{
        ...
        RUN(read_input(buffer,&msa,number_of_seqs,names_of_sequences,sequences_in_cluster));
...
int read_input(char* infile,struct msa** msa, int number_of_seqs, char** names_of_sequences, char** sequences_in_cluster)
{
	...
	type = FORMAT_FA;
        ...
        if(type == FORMAT_FA){
                ...
                RUN(read_fasta(b,&m,number_of_seqs,names_of_sequences,sequences_in_cluster));
...
int read_fasta( struct in_buffer* b,struct msa** m, int number_of_seqs, char** names_of_sequences, char** sequences_in_cluster)
{
        struct msa* msa = NULL;
        struct msa_seq* seq_ptr = NULL;
        //FILE* f_ptr = NULL;
	...
	for(nl = 0; nl< number_of_seqs; nl++){
		line = names_of_sequences[nl];
		line_len = strlen(line);
		if(msa->alloc_numseq == msa->numseq){
			RUN(resize_msa(msa));
		}
		seq_ptr = msa->sequences[msa->numseq];
		snprintf(seq_ptr->name ,MSA_NAME_LEN ,"%s",line);
		msa->numseq++;
	}
	for(nl = 0; nl< number_of_seqs; nl++){
		line = sequences_in_cluster[nl];
		line_len = strlen(line);
		seq_ptr = msa->sequences[nl];
		for(i = 0;i < line_len;i++){
